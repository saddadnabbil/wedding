...
add_action('plugins_loaded', 'wdp_check_elkit'); function wdp_check_elkit() { if(defined('ELKIT_VERSION')) : add_action('admin_notices', 'cek_elkit_is_active'); function cek_elkit_is_active() {
        ...
        </div><?php } else : $license_status = get_option( 'weddingpress_license_status' ); if ( isset( $license_status->license ) && $license_status->license == 'valid' ) : if ( did_action( 'elementor/loaded' ) ) { require WEDDINGPRESS_ELEMENTOR_PATH . '/base.php'; \WDP\Elementor\Base::instance(); require WEDDINGPRESS_ELEMENTOR_PATH . '/classes/functions.php'; } endif; require_once dirname( __FILE__ ) . '/plugin.php'; $license_status = get_option( 'weddingpress_license_status' ); if ( isset( $license_status->license ) && $license_status->license == 'valid' ) { if ( class_exists( 'woocommerce') && did_action( 'elementor/loaded' ) ) { require_once dirname( __FILE__ ) . '/addons/woocommerce/woocommerce-setup.php'; WDP_WooCommerce::instance(); include_once( WEDDINGPRESS_ELEMENTOR_PATH . '/elementor/woocommerce.php' );} $comment = get_option( 'comment_type' ); if ( $comment == 'commentkit' ) { include_once( WEDDINGPRESS_ELEMENTOR_PATH . '/addons/comment-kit/comment-kit.php' ); } elseif( $comment == 'commentkit2' ) { include_once( WEDDINGPRESS_ELEMENTOR_PATH . '/addons/comment-kit2/comment-kit2.php' ); } } new WeddingPressV3_Plugin(); endif; } if (did_action('elementor/loaded')) { $license_status = get_option( 'weddingpress_license_status' ); if ( isset( $license_status->license ) && $license_status->license == 'valid' ) : include_once( WEDDINGPRESS_ELEMENTOR_PATH . '/addons/elementor-widget-manager/elementor-widget-manager.php' ); Elementor_Widget_Manager::instance(); endif; } $license_status = get_option( 'weddingpress_license_status' ); if ( isset( $license_status->license ) && $license_status->license == 'valid' ) : add_action( 'init', 'updater');endif; function updater() { $license_key = trim( get_option( 'weddingpress_license' ) ); if ( ! $license_key ) { return; } if( ! is_admin() || ! class_exists( 'WeddingPress_Updater' ) ) {return; } $edd_updater = new WeddingPress_Updater( WEDDINGPRESS_ELEMENTOR_STORE, __FILE__, array( 'version'   => WEDDINGPRESS_ELEMENTOR_VERSION, 'license'   => $license_key, 'item_name' => WEDDINGPRESS_ELEMENTOR_NAME, 'author'    => 'WeddingPress', 'beta'      => false ) );}   
